...
The v4.0.0 release introduces a lazy DataFrame API and expression DSL, adds search, natural-language-to-SQL, runtime status, active-query management, and mTLS support, modernizes the runtime requirements, and bumps core dependencies to their latest major versions.
...
`nsql_generate_sql()` generates the SQL without running it — inspect or edit it, or run it through `client.query(...)` for Arrow-typed results instead of `nsql()`'s decoded JSON rows.
