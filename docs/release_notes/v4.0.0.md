...
Both calls are scoped to the authenticated principal (API key or client certificate), not to the `Client` instance, and reach only the one runtime process behind `http_url`. Runtime releases up to and including v2.1.5 don't scope either endpoint by credential at all.
...
**The DataFrame entry point is `Client.from_sql(query)`, not `Client.sql(query)`.** The DataFrame API above was developed alongside this release; `sql()` was its working name until the rename above freed it up for the synchronous-query method. If you tried the DataFrame API against an earlier v4.0.0 pre-release build, update `client.sql(query)` (DataFrame) to `client.from_sql(query)`.
